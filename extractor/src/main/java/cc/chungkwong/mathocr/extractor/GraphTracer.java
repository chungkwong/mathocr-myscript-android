...
public class GraphTracer {
    ...
    public static TraceList trace(Graph<Junction, Segment> graph) {
        ...
        double thick=ThinTracer.getAverageThick(graph.getEdges());
        ...
        for (Iterator<Graph<Junction, Segment>> iterator = graph.getComponents(); iterator.hasNext(); ) {
            ...
            for(Segment edge:new ArrayList<>(component.getEdges())){
                int t=edge.getTrace().getPoints().size();
                if(t*t<=thick/2){
                    if(componentBackup.getEdges().contains(edge)&&componentBackup.getEdges(componentBackup.getStart(edge)).size()==1&&componentBackup.getEdges(componentBackup.getEnd(edge)).size()==1){
                        traces.add(toDot(graph.getStart(edge)));
                    }
                    ThinTracer.removeEdge(edge,component);
                }
            }
    ...
    private static void traceDot(Graph<Junction, Segment> graph, List<Trace> traces) {
        for (Iterator<Junction> iterator = graph.getVertexs().iterator(); iterator.hasNext(); ) {
            ...
            if (graph.getEdges(vertex) == null || graph.getEdges(vertex).isEmpty()) {
                traces.add(toDot(vertex));
                iterator.remove();
            }
        }
    }
    private static Trace toDot(Junction vertex){
...
//			}
    ...
    private static void fixDouble(Map<Trace, Pair<Junction, Junction>> traceEnds, final Graph<Junction, Segment> graph,double thick) {
        ...
        Collections.sort(linkages, new Comparator<Segment>() {
            ...
            public int compare(Segment t0, Segment t1) {
                return Double.compare(turning.get(t0),turning.get(t1));
            }
        });
    ...
    private static boolean isRightAngle(Trace start, boolean startForward, Trace end, boolean endForward,double thick) {
        if(start.getPoints().size()*start.getPoints().size()<thick/2||end.getPoints().size()*end.getPoints().size()<thick/2){
            return false;
        }
