...
//                if (t<=thick / 2){//t <= edge.getThick() || t <= thick || t <= 9) {
                if(edge.getThick()<thick/4||t<=thick/16||(t<=thick/2&&graph.getEdges(graph.getStart(edge)).size()>1&&graph.getEdges(graph.getEnd(edge)).size()>1)){
    ...
    static int getAverageThick(Collection<Segment> segments) {
        if (segments.isEmpty())
            return 0;
        int thickSum = 0;
    ...
    static void removeEdge(Segment edge, Graph<Junction, Segment> graph) {
